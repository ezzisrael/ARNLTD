  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="grid h-14 w-14 place-items-center rounded-full bg-primary text-white shadow-lg"
      >
        {open ? <X /> : <MessageCircle />}
      </button>

      <div
        className={`mt-3 w-80 max-w-[92vw] overflow-hidden rounded-2xl shadow-2xl border transform transition-all ${open ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0 pointer-events-none"}`}
      >
